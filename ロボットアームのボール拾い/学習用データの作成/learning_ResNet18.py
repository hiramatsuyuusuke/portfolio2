#CIFAR10を「ロボットアームのボール拾い」用に書き換えて、ResNet18の学習を行います。
...
#「ロボットアームのボール拾い」用の画像を読み込んで画像データを変更し、ラベル番号を書き換える
j = 0   #while j < len( train_data):のカウント
i = 0   #読み込み画像番号のカウント
while j < len( train_data):
    #「ロボットアームのボール拾い」用の画像の読み込み
...
#「ロボットアームのボール拾い」用の画像を読み込んで画像データを変更し、ラベル番号を変書き換える
...
optimizer = optim.SGD(net.parameters(),lr=0.02,momentum=0.0,weight_decay=0.00005)
